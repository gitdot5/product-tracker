...
function AddForm({ onSave, onCancel, initialData }) {
    ...
    return (
        <div style={{ padding: 20, background: "#0e0e18", borderRadius: 12 }}>
            <h3 style={{ margin: "0 0 20px", color: "#fff" }}>{initialData ? "Edit Entry" : "Add New Entry"}</h3>
            <form onSubmit={handleSubmit}>
                ...
                <div style={fieldStyle}><label style={labelStyle}>Product</label><input value={formData.productName} onChange={e => handleChange("productName", e.target.value)} style={inputStyle} required placeholder="e.g. EpiFix" /></div>
                <div style={fieldStyle}><label style={labelStyle}>Item #</label><input value={formData.itemNumber} onChange={e => handleChange("itemNumber", e.target.value)} style={inputStyle} required placeholder="e.g. EF-123" /></div>
        ...
        <div style={{ background: S.bg, minHeight: "100vh", color: S.textMain, fontFamily: "Inter, sans-serif", paddingBottom: 40 }}>
            ...
            <div style={{ padding: "16px 20px 0" }}>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
                    ...
                    <button onClick={downloadCSV} style={{ ...S.pill, cursor: "pointer", fontSize: 12, fontWeight: 600 }}><svg width="14" height="14" viewBox="0 0 24 24" style={{ marginTop: -1 }}>{Icons.Csv}</svg> CSV</button>
                </div>
            </div>
            ...
            <div style={{ padding: "0 16px" }}>
                {tab === "products" && (
                    <>
                        <input placeholder="Search..." value={q} onChange={e => setQ(e.target.value)} style={{ width: "100%", padding: "12px 16px", borderRadius: 10, background: "transparent", border: "1px solid #2a2a35", color: "#fff", marginBottom: 10, fontSize: 13, outline: "none" }} />
                ...
                {tab === "add" && (
                    <AddForm initialData={editing} onSave={saveEntry} onCancel={() => { setTab("products"); setEditing(null); }} />
                )}
