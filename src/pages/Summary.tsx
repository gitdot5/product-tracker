...
export default function Summary() {
  ...
  return (
    <div className="page">
      ...
      {vendors.length > 0 && (
        <section>
          <h2 className="section-title">Vendors ({vendors.length})</h2>
          <ul className="card-list">
            {vendors.slice(0, 5).map((v) => (
              <li key={v.name} className="card card-compact">
                <div className="card-row">
                  <div className="card-info">
                    <span className="card-name">{v.name}</span>                    <span className="card-meta">
                      {v.count} {v.count === 1 ? "entry" : "entries"}
                    </span>
                  </div>
                  <div className="card-right">
                    <span style={{ fontWeight: 600 }}>${v.total.toFixed(2)}</span>
                  </div>
                </div>
              </li>
            ))}
        ...
        <section>
          ...
          <ul className="card-list">
            {entries.slice(0, 5).map((e) => (
              <li key={e.id} className="card card-compact">
                <div className="card-row">
                  <div className="card-info">
                    <span className="card-name">{e.product_name}</span>
                    <span className="card-meta">
                      {e.vendor} · {e.patient || "No patient"} · {new Date(e.date).toLocaleDateString()}
                    </span>
                  </div>
                  <div className="card-right">
                    <span style={{ fontWeight: 600 }}>${Number(e.cost).toFixed(2)}</span>                  </div>
                </div>
              </li>
            ))}
          </ul>
